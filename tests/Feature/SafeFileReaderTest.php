...
test('it handles JSON line-by-line decoding', function () {
    ...
    $reader = new SafeFileReader($this->tempFilePath);
    $jsonLines = iterator_to_array($reader->jsonLines(), false);
...
test('it throws exception on invalid JSON decoding', function () {
    ...
    $reader = new SafeFileReader($this->tempFilePath);
    expect(fn() => iterator_to_array($reader->jsonLines(), false))->toThrow(Exception::class);
});
...
test('it reads XML elements', function () {
    ...
    $elements = iterator_to_array($reader->xmlElements('item'), false);
...
test('it reads serialized data', function () {
    ...
    $reader = new SafeFileReader($this->tempFilePath);
    $serializedObjects = iterator_to_array($reader->serializedValues(), false);
...
test('it rejects serialized objects for safety', function () {
    ...
    expect(fn() => iterator_to_array($reader->serializedValues(), false))
...
test('it throws exception if file is not accessible', function () {
    ...
    expect(fn() => $reader->lines())->toThrow(FileAccessException::class);
});
...
test('it creates an independent line iterable for each read', function () {
    $reader = new SafeFileReader($this->tempFilePath);
    $first = iterator_to_array($reader->lines(), false);
    $second = iterator_to_array($reader->lines(), false);
